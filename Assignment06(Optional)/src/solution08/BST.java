...
public class BST {
	...
	public int[] children(int key, Node trav) {
		if (trav == null)
			return null;
		if (trav.data == key && trav.left != null && trav.right != null)
			return new int[] { trav.left.data, trav.right.data };
		else if (trav.data == key && trav.left != null && trav.right == null)
			return new int[] { trav.left.data };
		else if (trav.data == key && trav.left == null && trav.right != null)
			return new int[] { trav.right.data };
		else if (key < trav.data)
			return children(key, trav.left);
		else
			return children(key, trav.right);
	}

	public int[] children(int key) {
		return children(key, root);
	}

	public void printSibling(Node trav) {
		if (trav == null)
			return;
		int[] arr = children(trav.data);
		if (arr != null) {
			for (int ele : arr) {
		...
		printSibling(trav.left);
		printSibling(trav.right);
	}
